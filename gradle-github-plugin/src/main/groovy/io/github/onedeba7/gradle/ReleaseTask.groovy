...
class ReleaseTask extends DefaultTask {
    ...
    // header - constant, doesn't affect task inputs/outputs
    @Internal
    ...
    public void postAssets(String uploadUrl, def assets, String accept) {
        assets.each { asset ->
            ...
            if (file.exists()) {
                if (file.isDirectory()) {
                    def zipFile = new File(file.getParentFile(), file.getName() + ".zip")
                    ZipUtil.pack(file, zipFile)
                    file = zipFile
                }

                def map = URLConnection.getFileNameMap()
                def contentType = map.getContentTypeFor(file.getAbsolutePath())
